/* ──────────────────────────────────────────────────────────
   INCHARGE · tweaks.js
   Painel de Tweaks (vanilla) — Layout / Densidade / KPIs.
   ────────────────────────────────────────────────────────── */
(function () {
  const root = document.documentElement;

  // Defaults — synced with the EDITMODE-BEGIN block in index.html
  ...
  let state = { ...DEFAULTS };
  // Load from localStorage (so it survives reload in the preview too)
  try {
  ...
  fab.setAttribute("aria-label", "Abrir Tweaks");
  ...
  function renderPanel() {
    ...
    head.innerHTML = `<div class="tweaks__title">Tweaks</div>`;
    ...
    body.append(
      ...
      sw("Mostrar KPIs no topo", "showKpis"),
    );
  ...
  applyState();
  renderPanel();
  // Show FAB by default so users can find it in the preview
  fab.classList.add("is-visible");
